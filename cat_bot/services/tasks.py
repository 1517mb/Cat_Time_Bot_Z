...
async def send_leave_reminder_task(
    ...
    try:
        ...
        users_list_str = "\n".join(users)
        message = (
            "🐾 <b>КотБот напоминает!</b>\n\n"
            "Кажется, кто-то забыл закрыть свой выезд. "
            "Следующие сотрудники всё ещё на объектах:\n"
            f"{users_list_str}\n\n"
            "🛠 <b>Ваши действия:</b>\n"
            "1. <b>Уже ушли домой?</b> Закройте выезд задним числом: "
            "<code>/edit_end ЧЧ:ММ</code> (например, "
            "<code>/edit_end 18:00</code>).\n"
            "2. <b>Выходите прямо сейчас?</b> Жмите <code>/leave</code>.\n"
            "3. <b>Всё ещё работаете?</b> Тогда не отвлекаю, "
            "игнорируйте это сообщение! ☕️\n\n"
            "<i>💡 Ошиблись со временем приезда? "
            "Вас спасет <code>/edit_start ЧЧ:ММ</code>.</i>"
        )
        await bot.send_message(
            chat_id=chat_id,
            text=message,
