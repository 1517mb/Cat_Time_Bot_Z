    ...
    async with async_session_maker() as session:
        ...
        for i in range(1, 101):
            if i == 1:
                ...
                step = 80
            exp_current += step
            title, cat = LEVEL_TITLES_DATA.get(i, (f"Уровень {i}", "legend"))
            stmt = select(models.LevelTitle).where(
                models.LevelTitle.level == i)
            ...
            if obj:
                obj.title = title
                obj.category = cat
                obj.min_experience = exp_current
            else:
                obj = models.LevelTitle(
                    level=i,
                    title=title,
                    category=cat,
                ...
                session.add(obj)
        delete_stmt = delete(
            models.LevelTitle).where(models.LevelTitle.level > 100)
        await session.execute(delete_stmt)

        await session.commit()
        logger.info("✅ Успешно обновлено 100 уровней "
                    "(включая уникальные названия)!")
